...
): Promise<ProfileUserDTO> => {  // ✅ change return type to ProfileUserDTO
  try {
    ...
    // ✅ getUserProfileService already returns ProfileUserDTO now
    return await this.getUserProfileService(userId.toString());
  } catch (error) {
