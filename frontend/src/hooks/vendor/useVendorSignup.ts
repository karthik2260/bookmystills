...
import { VendorFormErrorValues, VendorFormValues } from '@/types/vendorTypes';
...
export const useVendorSignup = () => {
  ...
  const [formErrors, setFormErrors] = useState<VendorFormErrorValues>(initialErrorValues);
...
const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  setFormErrors((prev) => ({ ...prev, portfolioImages: errors.portfolioImages }));
...
const handleRemoveImage = (index: number) => {
  ...
  const handleAadharChange = (e: React.ChangeEvent<HTMLInputElement>, side: 'front' | 'back') => {
  ...
  if (Object.values(errors).every((error) => error === "")) {
    try {
      ...
      formValues.portfolioImages.forEach((file) => {
        ...
        if (formValues.aadharImages[0]) formData.append("aadharFront", formValues.aadharImages[0]);
        if (formValues.aadharImages[1]) formData.append("aadharBack", formValues.aadharImages[1]);
