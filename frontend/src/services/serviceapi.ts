...
import { VendorResponse } from '../types/vendorTypes';
import { PostData, PostStatus } from '@/types/postTypes';

export const fetchVendors = async (page: number, search: string): Promise<VendorResponse> => {
...
export const fetchUnavailableDatesAPI = async (): Promise<string[]> => {
  const response = await axiosInstance.get<UnavailableDatesResponse>('/dateAvailabilty');
  console.log('Date response:', response.data);

  return response.data.result?.bookedDates || [];
};

export const updateAvailabilityAPI = async (dates: string[], mode: 'block' | 'unblock') => {
