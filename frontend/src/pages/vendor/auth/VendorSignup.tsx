...
import { Sparkles, User, Mail, Phone, Lock, Building2, FileText, MapPin, Upload, X, ShieldCheck } from 'lucide-react';
import {
  Stepper,
  Step,
...
const VendorSignUp = () => {
  const {
    formValues, formErrors, images, imageIndex,
    handleChange, handleRemoveImage, submitHandler,
    activeStep, handleNext, handlePrev,
    isLastStep, isFirstStep, setIsLastStep, setIsFirstStep,
    handleFileChange,
    ...
    <div className="w-full h-screen flex overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10 space-y-4">
          <h1 className="text-4xl font-bold text-white leading-tight">
            Join as a<br />Trusted Vendor
          </h1>
          <p className="text-blue-100 text-base leading-relaxed max-w-xs">
            Showcase your services, attract clients, and grow your event business on our platform.
          </p>
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-gray-50 px-6 py-8 overflow-y-auto">
        <div className="w-full max-w-md space-y-5">
          ...
          {/* Header */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-blue-600 mb-1">Vendor Registration</p>
            <h2 className="text-3xl font-bold text-gray-900">Create your vendor account</h2>
            <p className="text-sm text-gray-500 mt-1">Complete both steps to get started</p>
          </div>
          ...
          >
            <Step placeholder={undefined} onPointerEnterCapture={() => {}} onPointerLeaveCapture={() => {}}>1</Step>
            <Step placeholder={undefined} onPointerEnterCapture={() => {}} onPointerLeaveCapture={() => {}}>2</Step>
          </Stepper>
          ...
          <div className="flex items-center gap-2">
            {steps.map((label, i) => (
              <div key={i} className="flex items-center gap-2 flex-1">
                ...
                <span className={`text-xs font-medium whitespace-nowrap ${i === activeStep ? 'text-blue-600' : 'text-gray-400'}`}>
                  {label}
                </span>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-px ${i < activeStep ? 'bg-blue-600' : 'bg-gray-200'}`} />
                )}
              </div>
            ))}
          </div>
              ...
              <>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input id="contactinfo" type="text" name="contactinfo" placeholder="+91 98765 43210" autoComplete="tel"
                      onChange={handleChange} value={formValues.contactinfo}
                  ...
                  {formErrors.contactinfo && <p className="text-xs text-red-500">{formErrors.contactinfo}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input id="confirm-password" type="password" name="confirmPassword" placeholder="••••••••" autoComplete="new-password"
                      onChange={handleChange} value={formValues.confirmPassword}
              ...
              <>
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input id="companyName" type="text" name="companyName" placeholder="Your Company" autoComplete="organization"
                      onChange={handleChange} value={formValues.companyName}
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <textarea id="about" name="about" placeholder="Tell clients about your services…"
                      onChange={(e) => handleChange(e as unknown as React.ChangeEvent<HTMLInputElement>)}
                      value={formValues.about} rows={3}
                  ...
                  {formErrors.about && <p className="text-xs text-red-500">{formErrors.about}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input id="city" type="text" name="city" placeholder="e.g. Mumbai" autoComplete="address-level2"
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <label className="text-sm font-medium text-gray-700">
                      Aadhaar Card
                      <span className="text-xs text-gray-400 ml-1 font-normal">(Front & Back)</span>
                    </label>
                  </div>
                  <p className="text-xs text-gray-400">Required for identity verification by admin. Images are kept confidential.</p>

                  <div className="grid grid-cols-2 gap-3">

                    {/* Front */}
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Front Side</label>
                      <label htmlFor="aadharFront"
                        ...
                        {formValues.aadharImages[0] ? (
                          <img src={URL.createObjectURL(formValues.aadharImages[0])} alt="Aadhaar Front"
                        ...
                        <input id="aadharFront" type="file" accept="image/*"
                          onChange={(e) => handleAadharChange(e, 'front')} className="hidden" />
                      </label>
                      {formValues.aadharImages[0] && (
                        <button type="button" onClick={() => handleRemoveAadhar('front')}
                    ...
                    {/* Back */}
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Back Side</label>
                      <label htmlFor="aadharBack"
                        className={`flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-lg cursor-pointer transition overflow-hidden
                          ${formValues.aadharImages[1]
                            ...
                            : 'border-gray-300 bg-white hover:border-blue-400 hover:bg-blue-50'}`}>
                        {formValues.aadharImages[1] ? (
                          <img src={URL.createObjectURL(formValues.aadharImages[1])} alt="Aadhaar Back"
                            className="w-full h-full object-cover" />
                        ) : (
                          <>
                            <Upload size={18} className="text-gray-400 mb-1" />
                        ...
                        <input id="aadharBack" type="file" accept="image/*"
                          onChange={(e) => handleAadharChange(e, 'back')} className="hidden" />
                      </label>
                      {formValues.aadharImages[1] && (
                        <button type="button" onClick={() => handleRemoveAadhar('back')}
                          className="text-xs text-red-500 hover:underline flex items-center gap-1">
                          <X size={10} /> Remove
                        </button>
                      )}
                  ...
                  {formValues.aadharImages[0] && formValues.aadharImages[1] ? (
                    <p className="text-xs text-green-600 flex items-center gap-1 font-medium">
                      <ShieldCheck size={12} /> Both sides uploaded — ready for admin verification
                    </p>
                  ) : (
                    formErrors.aadharImages && (
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">
                    Portfolio Images
                    <span className="text-xs text-gray-400 ml-1 font-normal">(Min 3, Max 5)</span>
                  </label>
                  <label htmlFor="portfolioImages"
                    ...
                    <input id="portfolioImages" type="file" name="portfolioImages"
                      multiple accept="image/*" onChange={handleFileChange} className="hidden" />
                  </label>

                  {formValues.portfolioImages.length > 0 && (
                    <p className="text-xs text-gray-500">✅ {formValues.portfolioImages.length} / 5 image(s) selected</p>
                  )}
                  ...
                  {formErrors.portfolioImages && (
                    <p className="text-xs text-red-500">{formErrors.portfolioImages}</p>
                  )}
                </div>
              </>
            )}
          ...
          <p className="text-center text-sm text-gray-500">
            ...
            <Link to={VENDOR.LOGIN} className="text-blue-600 font-semibold hover:underline">Login</Link>
          </p>
          <p className="text-center text-sm text-gray-500 pb-6">
            Want to register as a user?{' '}
            <Link to={USER.SIGNUP} className="text-blue-600 font-semibold hover:underline">Sign up here</Link>
          </p>
        </div>
      </div>
    </div>
