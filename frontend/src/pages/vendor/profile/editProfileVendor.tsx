  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasErrors) {
      try {
        ...
        if (formData.name !== vendor?.name)
          formDataToSend.append("name", formData.name);
        if (formData.contactinfo !== vendor?.contactinfo)
          formDataToSend.append("contactinfo", formData.contactinfo);
        if (formData.imageUrl)
          formDataToSend.append("image", formData.imageUrl);
        if (formData.companyName !== vendor?.companyName)
          formDataToSend.append("companyName", formData.companyName);
        if (formData.city !== vendor?.city)
          formDataToSend.append("city", formData.city);
        if (formData.about !== vendor?.about)
            ...
            >
              ...
              >
                {isLoading ? (
                  <>
                    <Loader2
                      className="animate-spin mr-2 bg-black text-white"
                      size={16}
                    />
                    Saving...
                  </>
