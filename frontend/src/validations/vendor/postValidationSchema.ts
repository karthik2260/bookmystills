...
const countValidImages = (
  ...
  const newValidImages = imageFiles.filter(
    (file): file is File => file instanceof File,
  ).length;

  // Count existing image URLs
  const existingValidImages = existingImages.filter(
    (url) => url && url !== "",
  ).length;
...
export const postValidationSchema = (
  ...
  return Yup.object().shape({
    ...
    images: Yup.array().test("image-requirements", function (value) {
      ...
      if (!isEditMode && totalImages > 6) {
        return this.createError({
          message: "Maximum 6 images allowed for new posts",
        });
      }
      ...
      if (invalidFile) {
        return this.createError({
          message:
            "All images must be JPG, JPEG, PNG, or WebP and less than 5MB",
        });
      }
